...
describe("Voting Contract", function () {
    let Voting, voting, owner, disputeId, reason
    ...
    it("Should initialize dispute for voting correctly", async function () {
        reason = "Test Reason"
        ...
        const totalDisputes = await voting.totalDisputes()
        expect(totalDisputes.toNumber()).to.equal(1)
    })

    it("Should get OngoingDispute voting correctly", async function () {
        const disputeProject = await voting.disputedProjects(disputeId)

        expect(disputeProject.disputeId).to.equal(disputeId)
        expect(disputeProject.reason).to.equal(reason)
        expect(disputeProject.totalVoters).to.equal(100)
        expect(disputeProject.disputeRaiser).to.equal(owner.address)
        expect(disputeProject.votesA).to.equal(0)
        expect(disputeProject.votesB).to.equal(0)
    })

    it("Should get the disputeIdToHash correctly", async function () {
        const disputeIdReason = await voting.disputeIdToHash(1)

        expect(disputeIdReason).to.equal(ethers.utils.id(reason))
    })

    it('Should enter the vote for the dispute correctly', async function () {
        const voteA = await voting.vote(disputeId, 1, 12532609583862916517n, 12532609583862916517n, voting.address, 32);

        await delay(25000) /// waiting 25 seconds for block confirmation.

        const disputeProject1 = await voting.disputedProjects(disputeId)

        expect(disputeProject1.disputeId).to.equal(disputeId)
        expect(disputeProject1.reason).to.equal(reason)
        expect(disputeProject1.totalVoters).to.equal(100)
        expect(disputeProject1.disputeRaiser).to.equal(owner.address)
        expect(disputeProject1.votesA).to.equal(1)
        expect(disputeProject1.votesB).to.equal(0)


        const voteB = await voting.vote(disputeId, 1, 12532609583862916517n, 12532609583862916517n, voting.address, 54);

        await delay(25000) /// waiting 25 seconds for block confirmation.

        const disputeProject2 = await voting.disputedProjects(disputeId)

        expect(disputeProject2.disputeId).to.equal(disputeId)
        expect(disputeProject2.reason).to.equal(reason)
        expect(disputeProject2.totalVoters).to.equal(100)
        expect(disputeProject2.disputeRaiser).to.equal(owner.address)
        expect(disputeProject2.votesA).to.equal(2)
        expect(disputeProject2.votesB).to.equal(0)


        const voteC = await voting.vote(disputeId, 1, 12532609583862916517n, 12532609583862916517n, voting.address, 62);
        ...
        expect(disputeProject.totalVoters).to.equal(100)
        expect(disputeProject.disputeRaiser).to.equal(owner.address)
        expect(disputeProject.votesA).to.equal(3)
        expect(disputeProject.votesB).to.equal(0)
    ...
    it('Should get the voting result correctly', async function () {
        const votingResult = await voting.getVotingResult(disputeId)
        expect(votingResult).to.equal(1)
    })
