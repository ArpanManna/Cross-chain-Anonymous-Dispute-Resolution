...
contract Voting is CCIPReceiver, VRFConsumerBaseV2, ConfirmedOwner {
    ...
    constructor(address _router, address _link) CCIPReceiver(_router) VRFConsumerBaseV2(getCordinator(_router)) ConfirmedOwner(msg.sender){
        link = _link;
        router = _router;
    ...
    function initializeVoting(uint _disputeId) public {
    ...
    function vote(uint _disputeId, uint _vote, uint64 sourceChainSelector, uint64 destinationChainSelector, address _receiver, uint _VRFdata) public {
    ...
    function _ccipReceive(Client.Any2EVMMessage memory message) internal override {
        (uint _disputeId, string memory _vote, uint randomness) = abi.decode(message.data, (uint, string, uint));
		...
		require(disputedProjects[_disputeId].duration > block.timestamp, "Voting for this project is Over!");
        if(compare(_vote, "A")){
    ...
    function getCordinator(address _router) public pure returns(address coordinator){
        if(_router == 0x554472a2720E5E7D5D3C817529aBA05EEd5F82D8){      // avalanche fuji
            return 0x2eD832Ba664535e5886b75D64C46EB9a228C2610;
        }
        else if(_router == 0x70499c328e1E2a3c41108bd3730F6670a44595D1){  // polygon mumbai
            return 0x7a1BaC17Ccc5b313516C5E16fb24f7659aA5ebed;
        }
        else if(_router == 0x9527E2d01A3064ef6b50c1Da1C0cC523803BCFF2){  // BNB chain
            return 0x6A2AAd07396B36Fe02a22b33cf443582f682c82f;
        }
        else if(_router == 0xD0daae2231E9CB96b94C8512223533293C3693Bf){  // sepolia 
    ...
    function requestRandomWords() external onlyOwner returns (uint256 requestId){
        requestId = COORDINATOR.requestRandomWords(getKeyHash(router), getSubscriptionId(router), requestConfirmations, callbackGasLimit, numWords);
